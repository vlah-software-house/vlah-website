...
export const article = {
  ...
  description:
    'Spinning up a fresh VM for AI-assisted development means installing Docker, Go, Node.js, Python, and Claude Code — then repeating the whole ritual every time. We built setup-claude-vm.sh: a single idempotent bash script that gets any Ubuntu 24.04 machine ready for Claude coding sessions in minutes, not hours.',
  author: {
    name: 'Dragos Vilcu',
    role: 'Co-Founder / CEO',
...
🚀 [View the artifact & grab the script on Claude](https://claude.ai/public/artifacts/655ba8c5-c12a-4697-a7ff-a90a67d93b03)

## What is setup-claude-vm.sh?

**`setup-claude-vm.sh`** is a single, self-contained bash script that configures a clean Ubuntu 24.04 LTS machine for Claude agent and AI-assisted coding sessions. It installs and wires together everything an AI coding session needs — from system packages all the way up to Claude Code itself.
...
Installs Docker CE from the official Docker repository — not the outdated `docker.io` package from Ubuntu's default repos. Adds your user to the `docker` group and enables the daemon on boot.
...
### 8. Claude Code
Runs the official native Claude Code installer — the Bun-based one from `claude.ai/install.sh`. If Claude Code is already installed, the installer updates it to the latest version.
...
Configures `unattended-upgrades` for nightly, automatic security and system package updates. Auto-reboot is deliberately **disabled** — the script sets up a login notification and `.bashrc` check instead so you're prompted to reboot manually.

### 10. Shell Initialisation
Writes `/etc/profile.d/claude-tools.sh` (for login shells) and appends to `~/.bashrc` (for interactive bash, tmux, and screen panes) — both protected by idempotency markers so they're never duplicated on re-runs.

### 11 & 12. Global CLAUDE.md
Generates a comprehensive `~/.claude/CLAUDE.md` on every run. This file is picked up by Claude Code before any project-level context and documents the exact installed versions, tool locations, usage commands, and agent session best practices — all dynamically populated.
...
When you're running Claude Code agents on ephemeral VMs — spot instances, CI runners, or cloud dev environments — you want to declare the desired state of the machine and let the script figure out what work actually needs to be done. That's exactly what this script does.
...
We'd love to hear how you're using it — and what tools you'd add to the stack.
